...
class Groups extends Transform {
  ...
  createFilter(opts) {
    ...
    if (typeof opts !== 'object') return function () {}
    ...
    if (typeof filterEntries !== 'function') {
      filterEntries = function (file) {
        ...
        return multimatch(file, opts.filter).length
      }
    }
    let basedir = opts.basedir || this.basedir
    return function (file) {
      if (filterEntries(file)) {
        return output || path.relative(basedir, file)
      }
    }
  }
